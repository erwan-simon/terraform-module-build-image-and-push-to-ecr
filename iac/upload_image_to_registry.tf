resource "terraform_data" "ecr_upload" {
  provisioner "local-exec" {
    ...
    command     = "/bin/bash upload_image_to_registry.sh \"${var.code_path}\" \"${data.aws_caller_identity.current.account_id}\" \"${aws_ecr_repository.main.name}\" \"${data.aws_region.current.name}\" \"${local.image_tag}\" \"${var.role_to_assume_arn}\" \"${local.docker_build_args}\" \"${var.dockerfile_path}\""
  }
